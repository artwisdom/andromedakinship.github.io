...
- Resolution stays capped at 1.6 million pixels. Desktop allows up to 1.5 device-pixel ratio; narrow screens allow 1.75 for a sharper orbital view. Low-power mode remains capped at 1.0. Mobile uses fewer stars and fewer volume samples. One compressed, locally hosted cloud map loads only after travel begins; no remote texture service, 3D framework, or postprocessing service is needed.
- The cinematic pass follows a curved galactic approach, then a late exponential descent toward one original ringed planet. The world is 1,000 times smaller relative to the galaxy than in the previous preview; a planet-relative render frame retains surface precision. The initial stellar-scale dive precedes the planet's reveal, which starts as a tiny distant world before the rapid final approach. Analytic sphere/ring intersections provide opaque surface shading, front/back ring placement, the planet's shadow on its rings, a nearby star, and atmospheric glow. Surface detail is procedural and uses fewer noise samples on smaller/lower-power devices. The last camera position is still 1.85 planet radii from its center on desktop and 3.15 on narrow screens; pointer orbit remains outside its surface.
...
- The storm front occupies roughly two fifths of the sphere above a dark charcoal crust. An original generated cloud map supplies fine natural detail, with filtered mipmaps, seam-resistant three-axis mapping, a raised deck, and a drifting upper veil. Cold blue-violet lighting and subdued rings create the darker orbital mood. The planet reveal now starts at 70% of the passage, with a longer overlap of galactic dust/stars; final orbital distances are unchanged. See `docs/storm-texture-source.md` for the exact generation prompt, native dimensions, and asset provenance.
- Orbital lightning is a tiny diffuse cloud glow, not a visible bolt. A seeded scheduler places one event at a time with varied onset, duration, strength, and position. Each 0.82-second slot contains a 0.70–0.725-second glow and a brief dark interval: approximately twice the previous event rate, with a patch active about 87% of the time somewhere in the storm region (not necessarily on the visible hemisphere). Weather masking excludes calm regions, and no planet-wide exposure change occurs. Storms use the existing render clock and compositing pass. Pause, reduced motion, and hidden-tab behavior freeze them. A failed texture load retains procedural weather, and late image completion cannot restart paused or destroyed animation. This is not a formal photosensitivity certification.
...
For local failure-mode checks only, start the preview with `ANDROMEDA_QA=1 npm run dev`. The query modes `?qa=no-js`, `?qa=no-webgl`, `?qa=offline-apps`, `?qa=missing-clouds`, and `?qa=reduced-motion` simulate failures/preferences in the local preview. They are not generated into the public homepage.

`?qa=accessibility` adds a localhost-only axe-core scan and a rerun button to any HTML page; the JSON result is in `#qa-audit-result`. Wait for `#qa-audit-run:not([disabled])` before reading it. `?qa=scene-debug` exposes graphics errors, sample pixels, draw calls, flight progress, and the active cloud event on the local canvas. Its freeze/sample controls hold one normal cloud glow for visual inspection, never force all clouds to illuminate. Flight-position buttons use ordinary page scrolling to inspect the galaxy, dive, distant world, approach, and orbit. Release the weather hold to check normal motion. These controls are development fixtures only. Automated passes do not replace manual assistive-technology testing or a legal/commercial review of the apps.
